...
**Always use `flowchart`, not `graph`.** The `graph` keyword is a legacy alias that does not enable HTML labels by default. This means `<br/>` in node labels renders as literal text when using `mmdc` to export PNG/SVG. `flowchart` supports all the same syntax and enables HTML labels automatically.
...
Use `<br/>` inside quoted labels (requires `flowchart`, not `graph`):

```mermaid
flowchart LR
    A["Cursor<br/>(JWT)"] --> B["Gateway<br/>(Cognito)"]
```
