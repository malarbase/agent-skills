...
- **Always use `flowchart`, never `graph`** - `graph` is the legacy alias. It does not enable HTML labels by default, so `<br/>` in node labels renders as literal text (e.g., "Cursor\n(JWT)") instead of a line break. `flowchart` enables HTML labels and supports all the same syntax. This is especially visible when rendering to PNG via `mmdc`.
- **Line breaks in node labels** - Use `<br/>` inside quoted labels: `A["Line 1<br/>Line 2"]`. This only works with `flowchart` (not `graph`). Never use literal `\n` — Mermaid doesn't interpret escape sequences in labels.
- **Breaking characters** - Avoid `{}` in comments, use proper escape sequences for special characters. Quote node labels that contain parentheses or special chars: `A["Label with (parens)"]`
